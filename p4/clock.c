          ...
          unsigned long num_pages, void **proc_table)
{
  ...
  unsigned long total_page_frame = 0;
  unsigned long total_runnable_process = 0;
  ...
  */
  do
  {
    ...
    if (feof(fp))
    {
      ...
      if (io_head != NULL)
      {
        wait_clock_io_completion(&fp, &io_head, &io_tail,
                              &runnable_head, &runnable_tail,
                              &free_head, &free_tail, &ram_head, 
                              &ram_tail, &global_timer, &pt, &ipt, 
                              &clock_hand, num_pages);
        continue;
      }
    }
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            num_references++;
            total_page_frame += num_ram(ram_head);
            total_runnable_process += num_runnable_proc(runnable_head);
            // if page hit, set reference bit to 1;
            result_pt->value->ref_bit = 1;
  ...
  statistics* stat = malloc(sizeof(struct statistics));
  stat->AMU = total_page_frame / global_timer;
  stat->ARP =  total_runnable_process / global_timer;
  stat->TMR = num_references;
  stat->TPI = num_pagefault;
  stat-> RTime = global_timer;
  printf("statistics data for clock algorithm \n");
  printf("Average Memory Utilization (AMU): %g \n", stat->AMU);
  printf("Average Runable Processes (ARP): %g \n", stat->ARP);
  printf("Total Memory References (TMR): %lu \n", stat->TMR);
  printf("Total Page Ins (TPI): %lu \n", stat->TPI);
  printf("Running Time (RTime): %lu \n", stat-> RTime);
}
                            ...
                            unsigned long* clock_hand, unsigned long num_pages)
{
  ...
  *global_timer += tmp->timer;
  ...
  if (*free_head == NULL)
  {
    ...
    node_ipt* current_node = find_ipt(ipt, *clock_hand);
    
    // if the use bit of current frame at clock hand is 0, replace the page in 
    // the frame, otherwise, inscrease clock hand by 1 and set use bit to 0 
    // until find the first next page frame with use bit 0.
    while(1){
      if(current_node->value->ref_bit == 1){
        current_node->value->ref_bit = 0;
        *clock_hand = (*clock_hand + 1) % num_pages;
        if(*clock_hand == 0) *clock_hand = num_pages;
        current_node = find_ipt(ipt, *clock_hand);
      }
      else{
        break;
      }
    ...
    page* current_page = current_node->value;
    char* key_out = get_key_pt(current_page->pid, current_page->vpn);
    
    delete_pt(pt, key_out);
    delete_ipt(ipt, *clock_hand);


    // modify the page _____________________________________________________________________________________
    current_page->pid = (*runnable_tail)->pid;
    current_page->vpn = (*runnable_tail)->blocked_vpn;

    // update ipt (no need in this case)

    // add the "new" page to the page table
    node_pt *entry_pt = create_entry_pt(current_page); // create entry
    add_to_pt(pt, entry_pt);

    // add to ipt
    node_ipt *entry_ipt = create_entry_ipt(current_page->ppn, current_page);
...
unsigned long num_runnable_proc(process* runnable_head){
  process* temp = runnable_head;
  unsigned long runnable_length = 0;
  while(temp != NULL){
    runnable_length++;
    temp = temp->runnable_next;
  }
  return runnable_length;
}